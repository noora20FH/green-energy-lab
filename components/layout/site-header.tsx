...
import { Leaf, Menu, ChevronDown, Sun, Wind, Battery, Zap } from "lucide-react";
        ...
        >
            ...
            <div className={cn("text-center md:text-left transition-all")}>
              ...
              <p
                className={cn(
                  ...
                  isScrolled
                    ? "h-0 opacity-0 mt-0"
                    : "h-auto opacity-100 mt-1 text-sm md:text-xl",
                )}
              >
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                {/* Gunakan MAP agar dinamis */}
                {researchAreas.map((area) => (
                  <DropdownMenuItem
                    key={area.id}
                    onClick={() => {
                      setActiveTab("research-areas");
                      setResearchAreaPage(area.id); // Mengirim 'solar', 'wind', dll
                    }}
                  >
                    {area.title}
                  </DropdownMenuItem>
          ...
          <div
            className={cn(
              "md:hidden absolute right-4 top-8",
              isScrolled && "top-3",
            )}
          >
            <Button
          ...
          <nav className="md:hidden py-4 space-y-1 border-t border-white/20 animate-in slide-in-from-top-2 mt-2">
            ...
            {isResearchMobileOpen && (
              <div className="bg-black/20 rounded-md mx-2 overflow-hidden">
                <Button
                  variant="ghost"
                  className="w-full justify-start text-green-100 pl-8 text-sm hover:bg-white/10"
                  onClick={() => {
                    setActiveTab("research-areas");
                    setResearchAreaPage("solar");
                    setMobileMenuOpen(false);
                  }}
                >
            ...
            <Button
              ...
              className={cn(
                "w-full justify-start text-white hover:bg-white/20",
                activeTab === "publications" && "bg-white/10",
              )}
              onClick={() => handleMobileNav("publications")}
            >
              Publications
            </Button>
            <Button
              ...
              className={cn(
                "w-full justify-start text-white hover:bg-white/20",
                activeTab === "bio" && "bg-white/10",
              )}
              onClick={() => handleMobileNav("bio")}
            >
              Bio
            </Button>
            <Button
